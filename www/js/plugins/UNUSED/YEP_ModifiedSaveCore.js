 ...
 * @param Data Column 1
 * @parent ---Info Window---
 * @desc The data to be displayed in data column 1.
 * @default empty, playtime, save count, gold count
 *
 * @param Data Column 2
 * @parent ---Info Window---
 * @desc The data to be displayed in data column 2.
 * @default location, variable 1, variable 2, variable 3
 *
 * @param Data Column 3
 * @parent ---Info Window---
 * @desc The data to be displayed in data column 3.
 * @default empty, variable 4, variable 5, variable 6
 *
 * @param Data Column 4
 * @parent ---Info Window---
 * @desc The data to be displayed in data column 4.
 * @default
 *
 * @param ---Vocabulary---
 * @default
 ...
 * @on YES
 * @off NO
 * @desc Show the load confirmation window when loading a save file?
 * @default true
 *
 * @param Load Text
 * @parent ---Confirmation---
 ...
 * @on YES
 * @off NO
 * @desc Show the save confirmation window when overwriting a save file?
 * @default true
 *
 * @param Save Text
 * @parent ---Confirmation---
 ...
 * @on YES
 * @off NO
 * @desc Show the save confirmation window when deleting a save file?
 * @default true
 *
 * @param Delete Text
 * @parent ---Confirmation---
...
Yanfly.Param.SaveMaxFiles = Number(p['Max Files'] || 24);
Yanfly.Param.SaveIconSaved = Number(p['Saved Icon'] || 231);
Yanfly.Param.SaveIconEmpty = Number(p['Empty Icon'] || 230);
Yanfly.Param.SavePop = p['Return After Saving'] !== undefined ? eval(String(p['Return After Saving'])) : false;
Yanfly.Param.SaveAutoIndex = p['Auto New Index'] !== undefined ? eval(String(p['Auto New Index'])) : true;

Yanfly.Param.SaveCmdLoad = String(p['Load Command'] || 'Load');
Yanfly.Param.SaveCmdSave = String(p['Save Command'] || 'Save');
Yanfly.Param.SaveCmdDelete = String(p['Delete Command'] || 'Delete');

Yanfly.Param.SaveHelpSelect = String(p['Select Help'] || 'Silakan pilih slot penyimpanan.');
Yanfly.Param.SaveLoadSelect = String(p['Load Help'] || 'Memuat data dari file game.');
Yanfly.Param.SaveSaveSelect = String(p['Save Help'] || 'Menyimpan progres game saat ini.');
Yanfly.Param.SaveDeleteSelect = String(p['Delete Help'] || 'Menghapus semua data dari file ini.');

Yanfly.Param.SaveDeleteSound = {
  name:   String(p['Delete Filename'] || 'Damage2'),
  volume: Number(p['Delete Volume'] || 100),
  pitch:  Number(p['Delete Pitch'] || 150),
  pan:    Number(p['Delete Pan'] || 0)
};

Yanfly.Param.SaveInfoTitle = p['Show Game Title'] !== undefined ? eval(String(p['Show Game Title'])) : true;
Yanfly.Param.SaveInfoInvalid = String(p['Invalid Game Text'] || 'File simpanan untuk game lain.');
Yanfly.Param.SaveInfoEmpty = String(p['Empty Game Text'] || 'Kosong');
Yanfly.Param.SaveMapDisplayName = p['Map Display Name'] !== undefined ? eval(String(p['Map Display Name'])) : true;
Yanfly.Param.SaveInfoPartyType = Number(p['Party Display'] || 2).clamp(0, 3);
Yanfly.Param.SaveInfoPartyY = String(p['Party Y Position'] || 'this.lineHeight() + Window_Base._faceHeight');
Yanfly.Param.SaveInfoActorName = p['Show Actor Names'] !== undefined ? eval(String(p['Show Actor Names'])) : true;
Yanfly.Param.SaveInfoActorNameSz = Number(p['Name Font Size'] || 20);
Yanfly.Param.SaveInfoActorLv = p['Show Actor Level'] !== undefined ? eval(String(p['Show Actor Level'])) : true;
Yanfly.Param.SaveInfoActorLvSz = Number(p['Level Font Size'] || 20);
Yanfly.Param.SaveInfoActorLvFmt = String(p['Level Format'] || '\\c[16]%1 \\c[0]%3');
Yanfly.Param.SaveInfoDataSz = Number(p['Data Font Size'] || 20);

Yanfly.Param.SaveInfoDataCol1 = String(p['Data Column 1'] || 'empty, playtime, save count, gold count').split(',');
Yanfly.Param.SaveInfoDataCol2 = String(p['Data Column 2'] || 'location, variable 1, variable 2, variable 3').split(',');
Yanfly.Param.SaveInfoDataCol3 = String(p['Data Column 3'] || 'empty, variable 4, variable 5, variable 6').split(',');
Yanfly.Param.SaveInfoDataCol4 = String(p['Data Column 4'] || '').split(',');
...
Yanfly.Param.SaveVocabLocation = String(p['Map Location'] || '');
Yanfly.Param.SaveVocabPlaytime = String(p['Playtime'] || 'Playtime:');
Yanfly.Param.SaveVocabSaveCount = String(p['Save Count'] || 'Total Saves:');
Yanfly.Param.SaveVocabGoldCount = String(p['Gold Count'] || '%1:');

Yanfly.Param.SaveTechSaveMode = String(p['Save Mode'] || 'auto').trim().toLowerCase();
Yanfly.Param.SaveTechLocalConfig = String(p['Local Config'] || 'config.rpgsave');
Yanfly.Param.SaveTechLocalGlobal = String(p['Local Global'] || 'global.rpgsave');
Yanfly.Param.SaveTechLocalSave = String(p['Local Save'] || 'file%1.rpgsave');
Yanfly.Param.SaveTechWebConfig = String(p['Web Config'] || 'RPG %1 Config');
Yanfly.Param.SaveTechWebGlobal = String(p['Web Global'] || 'RPG %1 Global');
Yanfly.Param.SaveTechWebSave = String(p['Web Save'] || 'RPG %1 File%2');

Yanfly.Param.SaveConfirmLoad = p['Load Confirmation'] !== undefined ? eval(String(p['Load Confirmation'])) : true;
Yanfly.Param.SaveConfirmLoadTx = String(p['Load Text'] || 'Apakah kamu yakin ingin memuat file ini?');
Yanfly.Param.SaveConfirmSave = p['Save Confirmation'] !== undefined ? eval(String(p['Save Confirmation'])) : true;
Yanfly.Param.SaveConfirmSaveTx = String(p['Save Text'] || 'Apakah kamu yakin ingin menimpa file ini?');
Yanfly.Param.SaveConfirmDel = p['Delete Confirmation'] !== undefined ? eval(String(p['Delete Confirmation'])) : true;
Yanfly.Param.SaveConfirmDelTx = String(p['Delete Text'] || 'Apakah kamu yakin ingin menghapus file ini?');
Yanfly.Param.SaveConfirmYes = String(p['Confirm Yes'] || 'Ya');
Yanfly.Param.SaveConfirmNo = String(p['Confirm No'] || 'Tidak');
...
Window_SaveAction.prototype.makeCommandList = function() {
    this.addCommand(this.getCommandName('load'), 'load', true);
    this.addCommand(this.getCommandName('save'), 'save', this.isSaveEnabled());
    this.addCommand(this.getCommandName('delete'), 'delete', true);
    this.addCommand('Kembali', 'cancel', true);
};
...
Scene_File.prototype.createActionWindow = function() {
    ...
    this._actionWindow.setHandler('load',   this.onActionSelect.bind(this));
    this._actionWindow.setHandler('save',   this.onActionSelect.bind(this));
    this._actionWindow.setHandler('delete', this.onActionSelect.bind(this));
    this._actionWindow.setHandler('cancel', this.popScene.bind(this));
};

Scene_File.prototype.onActionSelect = function() {
...
Scene_File.prototype.onListCancel = function() {
    this._listWindow.deactivate();
    this._actionWindow.activate();
    this._helpWindow.setText(Yanfly.Param.SaveHelpSelect);
};
...
Scene_File.prototype.onActionLoad = function() {
    var id = this.savefileId();
    if (!DataManager.loadSavefileInfo(id)) {
